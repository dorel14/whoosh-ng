...
- `EmbeddingConfig` — embedding provider, model, and multi-field vectorization
...
| `whoosh_modern.config.engines.embedding` | `EmbeddingEngine` — builds an `EmbeddingProvider` from the `embedding` config block |
